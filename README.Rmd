...
`mcstatsim` is a lightweight, functional-programming approach to Monte Carlo
statistical simulation. The whole study is driven by a **single higher-order
function**, `runsim()`, and the package ships ready-to-use **performance
measures**, an **evaluation layer** that turns raw replicates into a tidy
results table, and **diagnostic plots**.
...
**Zip plot** (Morris, White & Crowther, 2019) -- confidence intervals sorted by
...
misses concentrated on one side, the signature of skew -- and climb back toward
0.95 as `n` grows. The normal-data intervals sit on 0.95 throughout.
...
- **Functional programming approach** -- one higher-order function drives the study.
