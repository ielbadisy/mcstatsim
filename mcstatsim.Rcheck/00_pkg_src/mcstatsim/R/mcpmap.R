#' Parallel Map Function using functionals::fmapn
#'
...
#' @details The function ensures that all elements in the list have the same length and uses `functionals::fmapn` for parallel processing.
...
mcpmap <- function(lists, func, num_cores = parallel::detectCores() - 1, show_progress = TRUE) {
  ...
  functionals::fmapn(lists, func, ncores = num_cores, pb = show_progress)
}
