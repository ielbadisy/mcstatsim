...
The core `runsim()` function expands the simulation grid across
replications and maps the resulting tasks to the simulation function.

Outputs are deliberately structured as data frames to simplify analysis
and visualization, addressing the limitations of list outputs in data
manipulation.

This package is designed for heavy Monte Carlo simulations. For small
jobs, process-based parallelism can be slower than sequential execution
because cluster startup and serialization overhead dominate.
...
in parallel using `mcstatsim`. The `show_progress = TRUE` argument keeps
an interactive text progress bar visible while tasks are being executed.
...
The table below summarizes a `bench::mark()` comparison between the
current base-`parallel` backend and two common R-level alternatives on a
...
The current release uses a process-based backend built on base
`parallel::makeCluster()` and `parallel::parLapply()`. This keeps the API
in R, supports arbitrary user-supplied simulation functions, and works
across operating systems. It also preserves a visible progress bar for
long-running jobs.
