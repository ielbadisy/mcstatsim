...
(5) Use the following simulation targets to compute the distortion between $\beta_{true}$ from `data_complete` and $\beta_i$ from `data_imputed`: 
...
_NB_: All these metrics (and more) are already implemented in the `mcstatsim` package (see `?calc_bias()`, `?calc_coverage()`, and `?calc_rmse()`.
